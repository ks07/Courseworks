...
\end{align*}
This preprocessing step is \(O(n)\). The set \(T\) can then be used to build a 4-dimensional data structure as described in subsection \ref{subsec:ddrangecount}, where each min/max coordinate is treated as a different dimension. Thus, the overall preprocessing is \(O(n \log^3 n)\), with space of \(O(n \log^3 n)\).

\subsubsection{Query Algorithm}

The query algorithm used here is identical to that of the d-dimensional point solution, where \(d=4\), and \(B = (x_1, x_2, y_1, y_2, x_1, x_2, y_1, y_2)\). For the avoidance of confusion, throughout this section the dimensions will be labelled \(s\), \(t\), \(u\), and \(v\), representing \(x_{min}\), \(y_{min}\), \(x_{max}\), and \(y_{max}\) respectively.
