...
m < \frac{1}{2}
\end{align*}
This contradiction thus shows it is impossible to have more than \(1+ \sqrt k\) mismatches in a given position without the total cost exceeding \(k\), when \(k=m\).

In order to maximise the number of mismatches without exceeding \(k\), they must be spaced evenly throughout the pattern, with the first error at position \(0\) and the last at position \(m-1\). With \(e = \sqrt k\) mismatches, the average distance between errors will be \(l_{avg} = \frac{m-1}{e-1}\) (\(1\) is subtracted from \(m\) and \(e\) to account for the first error in position 0). This means the average cost will equate to
\begin{algin*}
c_{avg} = \frac{m}{l_{avg}} \\ 
c_{avg} = \frac{m}{\frac{m-1}{e-1}}
c_{avg} = \frac{m(e-1)}{m-1}
c_{avg} = \frac{m(\sqrt m - 1)}{m-1}
TC = e * c_{avg}
TC = \frac{(\sqrt m) * m(\sqrt m - 1)}{m-1}
