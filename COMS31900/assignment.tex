...
the AB subset as \(Opt_{AB}\), and similarly \(Opt_C\) for the C subset. Running
FFD on these subsets will produce an \(s_x (x = AB | C)\) that satisfies:
\(Opt_{AB} <= s_{AB} <= (3/2) * Opt_{AB}\) and \(Opt_C <= s_C <= (3/2) *
Opt_C\). Combining the two approximations gives an inequality for the overall
approximation:
...
\(Opt_C <= Opt\). This can be explained by considering that C is a subset of N,
...
in this case, \(Opt_C <= Opt\), as removing items from the input cannot
...
limit. Thus it follows that \(Opt_{AB} <= Opt\). This can be used to express
