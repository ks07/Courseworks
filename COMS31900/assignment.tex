...
y_1 \leq L
\intertext{From the binary tree definitions, all nodes to the left of split must have \(y\) values that do not exceed \(y_2\). Thus, at each node \(Current\) in the search path, it can be shown that there are two possibilities based upon the value of \(y_{Current}\), i.e.}
\forall Current \in LeftPath :
\begin{cases}
\forall L \in Current_{Left}  : y_L   <  y_1          & \text{if } y_{Current} < y_1 \\
\forall R \in Current_{Right} : y_1 \leq y_R \leq y_2 & \text{if } y_{Current} \geq y_1
\end{cases}
